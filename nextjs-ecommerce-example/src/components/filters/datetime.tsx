...
export const DatetimeFilter: FC<{
  ...
  const [fromStable, setFromStable] = useState<{
    min?: string
    max?: string
  } | null>(null)
  const [toStable, setToStable] = useState<{
    min?: string
    max?: string
  } | null>(null)

  useEffect(() => {
    if (!fromStable && fromData)
      setFromStable({ min: fromData.min, max: fromData.max })
  }, [fromStable, fromData])
  useEffect(() => {
    if (!toStable && toData) setToStable({ min: toData.min, max: toData.max })
  }, [toStable, toData])

  const fromRange = fromStable ?? fromData
  const toRange = toStable ?? toData
              ...
              {isFromLoading ? (
                ...
                <>
                  <Calendar
                    mode="single"
                    defaultMonth={
                      fromRange.min && fromRange.max
                        ? new Date(
                            (new Date(fromRange.min).getTime() +
                              new Date(fromRange.max).getTime()) /
                              2
                          )
                        : new Date()
                    }
                    ...
                    disabled={
                      fromRange.min && fromRange.max
                        ? {
                            before: new Date(fromRange.min),
                            after: new Date(fromRange.max),
                          }
                        : undefined
                    }
              ...
              {isToLoading ? (
                ...
                <>
                  <Calendar
                    mode="single"
                    defaultMonth={
                      toRange.min && toRange.max
                        ? new Date(
                            (new Date(toRange.min).getTime() +
                              new Date(toRange.max).getTime()) /
                              2
                          )
                        : new Date()
                    }
                    ...
                    disabled={
                      toRange.min && toRange.max
                        ? {
                            before: new Date(toRange.min),
                            after: new Date(toRange.max),
                          }
                        : undefined
                    }
